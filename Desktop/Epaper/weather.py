...
class Weather:
    def __init__(self, latitude, longitude, api_id):
        ...
        self.data = requests.get(
            f"https://api.openweathermap.org/data/2.5/onecall?lat={self.latitude}&lon={self.longitude}&lang=pl&appid={self.api_key}").json()
    ...
    def weather_description(self, id):
        icon = "sun"
        weather_detail = "Słonecznie"
        if id // 100 != 8:
            id = id // 100
            if id == 2:
                icon = "thunder"
                weather_detail = "Burza"
            elif id == 3:
                icon = "drizzle"
                weather_detail = "Mżawka"
            elif id == 5:
                icon = "rain"
                weather_detail = "Deszcz"
            elif id == 6:
                icon = "snow"
                weather_detail = "Śnieg"
            elif id == 7:
                icon = "atm"
                weather_detail = "Mgła"
            else:
            ...
            if id == 801:
                icon = "25_clouds"
                weather_detail = "Chmurnie"
            elif id == 802:
                icon = "50_clouds"
                weather_detail = "Pochmurnie"
            elif id == 803 or id == 804:
                icon = "100_clouds"
                weather_detail = "Zachmurzenie"
